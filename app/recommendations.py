...
def get_recs(steam_id, number):
    ...
    # print(tag_list)
    user_vector = numpy.array([0.0] * len(tag_list))
    for i in range(len(games)):
        if games[i] not in [game["app_id"] for game in select_query("SELECT app_id FROM games")]:
            continue
    ...
    user_vector /= numpy.linalg.norm(user_vector)
    # print(tag_list)
    print(user_vector)

    recs = {}
    for game in [g["app_id"] for g in select_query("SELECT app_id FROM games")]:
        if game in games:
            continue
        game_vector = numpy.array([float(i) for i in select_query("SELECT tag_vector FROM games WHERE app_id=?", [game])[0]["tag_vector"].split(",")])
        distance = numpy.linalg.norm(user_vector - game_vector)
        if len(recs) < number:
            recs[game] = distance
            continue
